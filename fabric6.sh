...
while IFS=',' read -ra ADDR; do
    for i in "${ADDR[@]}"; do
        ...
        if [ -z "$zookeeperConnect" ]; then
            ...
            iiBOOTSTRAP_SERVERS="$i:9093"
        else
            iiBOOTSTRAP_SERVERS="$iiBOOTSTRAP_SERVERS,$i:9098"
...
while IFS=',' read -ra ADDR; do
    for i in "${ADDR[@]}"; do
        ...
        ssh -i $ssh_key $ssh_usr@$i "$command"
        echo 'sed -i "s@#CONSISTENCY_LEVEL=.*@CONSISTENCY_LEVEL=LOCAL_ONE@"   $K2_HOME/config/config.ini' >> fabric_$i.sh
        echo "sed -i "\"s@#USER=.*@USER=k2admin@\"" $K2_HOME/config/config.ini" >> fabric_$i.sh
        ...
        if [[ ! -z "$cassandra_DC2_IP" ]]; then
            ...
            echo "sed -i 's@#IIF_REPLICATION_OPTIONS=.*@IIF_REPLICATION_OPTIONS={ $class : $NetworkTopologyStrategy, $inside}@' $K2_HOME/config/iifConfig.ini" >> fabric_$i.sh
	...
	if [[ ! -z "$BOOTSTRAP_SERVERS" ]]; then
        ...
        #iifconfig
        sed1='"s@#KAFKA_BOOTSRTAP_SERVERS=.*@KAFKA_BOOTSRTAP_SERVERS='$iiBOOTSTRAP_SERVERS@'"'
        echo "sed -i "$sed1" $K2_HOME/config/iifConfig.ini" >> fabric_$i.sh
        sed2='"s@#ZOOKEEPER_BOOTSRTAP_SERVERS=.*@ZOOKEEPER_BOOTSRTAP_SERVERS='$zookeeperConnect'"'
        echo "sed -i  "$sed1" $K2_HOME/config/iifConfig.ini" >> fabric_$i.sh
